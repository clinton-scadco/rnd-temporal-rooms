...
use super::seed::Seed;
use super::{p3, spin_for, Mm, Piece, Vol, CLOSE, FAR, MEDIUM, P3, SIX};
...
pub fn run(d: &Design, plan: &Plan, seed: &Seed) -> Vec<Run> {
    ...
    for &i in &order {
        ...
        let r = one(&mut g, a, sa, b, sb, dom, name);
        done.push((i, r));
    }
...
fn one(
    ...
    Run { name, dom, bore: sa.bore.max(sb.bore), path, length, bends, props, direct }
}
...
pub fn dress(r: &Run, seed: &Seed, id: u16, out: &mut Vec<Piece>) {
    let t = treat(r.dom);
    let od = (r.bore * t.wide / 100).max(90);
    let bend = (od * 3) / 2;
    ...
    for i in 1..r.path.len() {
        ...
        // Make room for the elbows this segment runs into.
        let head = i > 1 && seg.is_axis() && r.path[i - 1].sub(r.path[i - 2]).is_axis();
        let tail = i + 1 < r.path.len() && seg.is_axis() && r.path[i + 1].sub(b).is_axis();
        if t.elbow && head && d > bend * 2 {
            a = a.add(unit_mm(seg, bend));
        }
        if t.elbow && tail && d > bend * 2 {
    ...
    for i in 1..r.path.len().saturating_sub(1) {
        ...
        if t.elbow && u.len() > bend * 2 && v.len() > bend * 2 {
            ...
            // A mitre: two stubs and no pretending. Shafts and conduit do this.
    ...
    if r.dom == Domain::Fluid && r.length > 6000 && rng.chance(70) {
        if let Some(at) = r.props.first() {
            out.push(
                Piece::new(Mesh::Valve, Mat::Paint, *at, super::EAST, p3(od * 2, od * 2, od * 2))
                    .lod(CLOSE),
            );
        }
    }
